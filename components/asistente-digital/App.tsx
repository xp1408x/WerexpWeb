...
function AsistenteDigitalApp() {
  ...
  const handleDetailsSubmit = useCallback(async (details: string, email: string) => {
    const formData = {
      idea: userIdea,
      solucion_titulo: selectedOption?.title,
      solucion_descripcion: selectedOption?.description,
      email: email,
      detalles: details,
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(responseData.errors?.map((e: any) => e.message).join(', ') || 'Error al enviar el formulario.');
      }
    } catch (err) {
